...
function ProductCard({ product, onAdd, onPlan, quantity }: { product: Product; onAdd: () => void; onPlan: () => void; quantity: number }) {
  return <article className={cn("group border bg-brand-cream transition-colors", quantity > 0 ? "border-brand-green" : "border-brand-deep/15")}><FoodImage type={product.visual} alt={`${product.name} breakfast`} /><div className="p-5"><div className="flex items-start justify-between gap-3"><h3 className="font-display text-2xl text-brand-deep">{product.name}</h3>{quantity > 0 && <span className="inline-flex shrink-0 items-center gap-1 text-xs font-bold text-brand-green"><Check className="size-4" /> {quantity}</span>}</div><p className="mt-3 min-h-[72px] text-sm leading-6 text-brand-ink/65">{product.description}</p><div className="space-y-2 border-t border-brand-deep/10 pt-4 text-xs text-brand-ink/60"><p><span className="font-bold text-brand-deep">Ingredients</span> {product.ingredients}</p><p><span className="font-bold text-brand-deep">Portion</span> {product.portion}</p></div><p className="mt-5 text-sm font-bold text-brand-deep">{formatPrice(product.price)}</p><div className="mt-4 flex flex-col gap-2"><Button className="w-full bg-brand-deep text-primary-foreground hover:bg-brand-green" onClick={onAdd}>{quantity > 0 ? `Add another · ${quantity}` : "Add to order"} <Plus /></Button><Button variant="ghost" className="w-full text-brand-deep hover:bg-brand-sage" onClick={onPlan}>Make it a plan <ArrowRight /></Button></div></div></article>;
}
...
function CartDrawer({ open, onClose, cart, cartCount, cartSubtotal, updateQuantity, onCheckout }: { open: boolean; onClose: () => void; cart: Cart; cartCount: number; cartSubtotal: number; updateQuantity: (id: string, quantity: number) => void; onCheckout: () => void }) {
  if (!open) return null;
  return <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label="Your cart"><button className="absolute inset-0 bg-brand-deep/35" onClick={onClose} aria-label="Close cart" /><aside className="absolute right-0 top-0 flex h-full w-full max-w-md flex-col border-l border-brand-deep/10 bg-brand-cream text-brand-ink shadow-soft"><div className="flex items-start justify-between border-b border-brand-deep/10 px-5 py-5"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-brand-green">Your order</p><h2 className="mt-1 font-display text-3xl text-brand-deep">Cart <span className="font-sans text-sm font-semibold text-brand-ink/55">({cartCount} items)</span></h2></div><Button variant="ghost" size="icon" className="text-brand-deep" onClick={onClose} aria-label="Close cart"><X /></Button></div><div className="flex-1 overflow-y-auto px-5 py-5">{cartCount === 0 ? <div className="flex h-full flex-col items-center justify-center text-center"><ShoppingBag className="size-9 text-brand-green" /><h3 className="mt-4 font-display text-2xl text-brand-deep">Your cart is empty</h3><p className="mt-2 max-w-xs text-sm leading-6 text-brand-ink/60">Add a breakfast from the menu and it will appear here.</p></div> : <div className="space-y-4">{products.filter((product) => (cart[product.id] ?? 0) > 0).map((product) => <CartLine key={product.id} product={product} quantity={cart[product.id] ?? 0} updateQuantity={updateQuantity} />)}</div>}</div><div className="border-t border-brand-deep/10 px-5 py-5"><div className="flex items-center justify-between text-base font-bold text-brand-deep"><span>Menu subtotal</span><span>{formatPrice(cartSubtotal)}</span></div><p className="mt-2 text-xs text-brand-ink/55">Routine, offer, and delivery details are confirmed next.</p><Button className="mt-5 h-12 w-full bg-brand-deep text-primary-foreground hover:bg-brand-green" disabled={cartCount === 0} onClick={onCheckout}>Complete your order <ArrowRight /></Button></div></aside></div>;
}

function CartLine({ product, quantity, updateQuantity }: { product: Product; quantity: number; updateQuantity: (id: string, quantity: number) => void }) { return <div className="flex gap-3 border-b border-brand-deep/10 pb-4"><div className="size-16 shrink-0 overflow-hidden border border-brand-deep/10"><img src={{ fruit: fruitAsset.url, sprouts: sproutsAsset.url, juice: juiceAsset.url, veggies: veggiesAsset.url }[product.visual]} alt={product.name} className="size-full object-cover" /></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-brand-deep">{product.name}</p><p className="mt-1 text-xs text-brand-ink/55">{formatPrice(product.price)} each</p></div><button className="text-xs font-semibold text-brand-ink/55 underline-offset-2 hover:text-brand-green hover:underline" onClick={() => updateQuantity(product.id, 0)}>Remove</button></div><div className="mt-3 flex items-center justify-between"><div className="inline-flex items-center border border-brand-deep/15"><Button variant="ghost" size="icon" className="size-8 rounded-none text-brand-deep" onClick={() => updateQuantity(product.id, quantity - 1)} aria-label={`Decrease ${product.name}`}><Minus className="size-3.5" /></Button><span className="w-8 text-center text-sm font-semibold text-brand-deep">{quantity}</span><Button variant="ghost" size="icon" className="size-8 rounded-none text-brand-deep" onClick={() => updateQuantity(product.id, quantity + 1)} aria-label={`Increase ${product.name}`}><Plus className="size-3.5" /></Button></div><span className="font-semibold text-brand-deep">{formatPrice((product.price ?? 0) * quantity)}</span></div></div></div>; }

function CheckoutDialog({ open, onOpenChange, step, plan, cart, selectedProducts, selectedProductIds, toggleProduct, updateQuantity, cartSubtotal, onPlanChange, deliveryDate, setDeliveryDate, dateLabel, customer, updateCustomer, nextStep, previousStep, savings }: { open: boolean; onOpenChange: (open: boolean) => void; step: CheckoutStep; plan: (typeof plans)[number]; cart: Cart; selectedProducts: Product[]; selectedProductIds: string[]; toggleProduct: (id: string) => void; updateQuantity: (id: string, quantity: number) => void; cartSubtotal: number; onPlanChange: (id: PlanId) => void; deliveryDate: Date | undefined; setDeliveryDate: (date: Date | undefined) => void; dateLabel: string; customer: { name: string; mobile: string; address: string; pin: string; landmark: string; window: string }; updateCustomer: (key: keyof typeof customer, value: string) => void; nextStep: () => void; previousStep: () => void; savings: ReturnType<typeof getSavings> }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto border-brand-deep/15 bg-brand-cream p-0 text-brand-ink"><div className="border-b border-brand-deep/10 px-5 pb-5 pt-6 sm:px-8"><DialogHeader className="text-left"><DialogTitle className="font-display text-3xl text-brand-deep">Complete your order</DialogTitle><DialogDescription className="text-brand-ink/60">Your cart stays updated as you choose your routine and delivery details.</DialogDescription></DialogHeader><div className="mt-6 grid grid-cols-3 gap-2">{[[1, "Breakfast"], [3, "Delivery"], [5, "Review"]].map(([value, label]) => <div key={label as string} className="flex items-center gap-2 text-xs font-bold"><span className={cn("flex size-6 items-center justify-center rounded-full border", step >= (value as number) ? "border-brand-green bg-brand-green text-primary-foreground" : "border-brand-deep/20 text-brand-ink/40")}>{step > (value as number) ? <Check className="size-3" /> : value}</span><span className={cn(step >= (value as number) ? "text-brand-deep" : "text-brand-ink/40")}>{label}</span></div>)}</div></div><div className="px-5 py-6 sm:px-8">{step === 1 && <CheckoutProducts cart={cart} selectedProductIds={selectedProductIds} toggleProduct={toggleProduct} updateQuantity={updateQuantity} />}{step === 2 && <CheckoutPlan plan={plan} onPlanChange={onPlanChange} />}{step === 3 && <CheckoutDate deliveryDate={deliveryDate} setDeliveryDate={setDeliveryDate} dateLabel={dateLabel} />}{step === 4 && <CheckoutDetails customer={customer} updateCustomer={updateCustomer} />}{step === 5 && <CheckoutReview plan={plan} selectedProducts={selectedProducts} cart={cart} cartSubtotal={cartSubtotal} dateLabel={dateLabel} customer={customer} savings={savings} />}</div><div className="flex flex-col-reverse gap-2 border-t border-brand-deep/10 px-5 py-4 sm:flex-row sm:justify-between sm:px-8"><Button variant="ghost" className="text-brand-deep" onClick={step === 1 ? () => onOpenChange(false) : previousStep}>{step === 1 ? "Keep browsing" : "Back"}</Button><Button className="bg-brand-deep text-primary-foreground hover:bg-brand-green" onClick={nextStep}>{step === 5 ? "Payment setup pending" : step === 4 ? "Review my order" : "Continue"}{step < 5 && <ArrowRight />}</Button></div></DialogContent></Dialog>;
}

function CheckoutProducts({ cart, selectedProductIds, toggleProduct, updateQuantity }: { cart: Cart; selectedProductIds: string[]; toggleProduct: (id: string) => void; updateQuantity: (id: string, quantity: number) => void }) { return <div><CheckoutHeading step="01" title="What would you like?" description="Choose breakfasts and adjust quantities before continuing." /><div className="mt-6 grid gap-3 sm:grid-cols-2">{products.map((product) => { const quantity = cart[product.id] ?? 0; return <div key={product.id} className={cn("flex items-center gap-3 border p-3 transition-colors", selectedProductIds.includes(product.id) ? "border-brand-green bg-brand-sage/55" : "border-brand-deep/15 hover:border-brand-green/50")}><button onClick={() => toggleProduct(product.id)} className="size-16 shrink-0 overflow-hidden border border-brand-deep/10" aria-label={`${quantity > 0 ? "Remove" : "Add"} ${product.name}`}><img src={{ fruit: fruitAsset.url, sprouts: sproutsAsset.url, juice: juiceAsset.url, veggies: veggiesAsset.url }[product.visual]} alt={product.name} className="size-full object-cover" /></button><div className="min-w-0 flex-1"><p className="font-semibold text-brand-deep">{product.name}</p><p className="mt-1 text-xs text-brand-ink/55">{formatPrice(product.price)}</p>{quantity > 0 && <div className="mt-2 inline-flex items-center border border-brand-deep/15"><Button variant="ghost" size="icon" className="size-7 rounded-none text-brand-deep" onClick={() => updateQuantity(product.id, quantity - 1)} aria-label={`Decrease ${product.name}`}><Minus className="size-3" /></Button><span className="w-7 text-center text-xs font-semibold text-brand-deep">{quantity}</span><Button variant="ghost" size="icon" className="size-7 rounded-none text-brand-deep" onClick={() => updateQuantity(product.id, quantity + 1)} aria-label={`Increase ${product.name}`}><Plus className="size-3" /></Button></div>}</div>{quantity > 0 ? <Check className="size-4 shrink-0 text-brand-green" /> : <Plus className="size-4 shrink-0 text-brand-ink/45" />}</div>; })}</div></div>; }
...
function CheckoutReview({ plan, selectedProducts, cart, cartSubtotal, dateLabel, customer, savings }: { plan: (typeof plans)[number]; selectedProducts: Product[]; cart: Cart; cartSubtotal: number; dateLabel: string; customer: { name: string; mobile: string; address: string; pin: string; landmark: string; window: string }; savings: ReturnType<typeof getSavings> }) { const offerDiscount = getOfferDiscount(plan); const baseTotal = plan.price === null ? null : plan.price + cartSubtotal; const total = baseTotal === null ? null : Math.max(baseTotal - offerDiscount + (storefrontConfig.deliveryFee ?? 0), 0); return <div><CheckoutHeading step="05" title="Review your order" description="Here is everything before payment is connected." /><div className="mt-6 space-y-3 border-y border-brand-deep/15 py-5 text-sm"><div><p className="font-semibold text-brand-deep">Chosen breakfasts</p><div className="mt-2 space-y-2 text-brand-ink/60">{selectedProducts.map((product) => <div className="flex justify-between gap-4" key={product.id}><span>{product.name} × {cart[product.id] ?? 0}</span><span>{formatPrice((product.price ?? 0) * (cart[product.id] ?? 0))}</span></div>)}</div></div><div className="flex justify-between gap-4 border-t border-brand-deep/10 pt-3"><span>Menu subtotal</span><span className="font-semibold">{formatPrice(cartSubtotal)}</span></div><div className="flex justify-between gap-4"><span>{plan.name} · {plan.deliveries} {plan.deliveries === 1 ? "delivery" : "deliveries"}</span><span className="font-semibold">{formatPrice(plan.price)}</span></div>{savings && savings.amount > 0 && <div className="flex justify-between gap-4 text-brand-green"><span>Plan savings included</span><span>-{formatPrice(savings.amount)}</span></div>}{offerDiscount > 0 && <div className="flex justify-between gap-4 text-brand-green"><span>{storefrontConfig.offer.label}</span><span>-{formatPrice(offerDiscount)}</span></div>}<div className="flex justify-between gap-4"><span>{storefrontConfig.deliveryLabel}</span><span>{storefrontConfig.deliveryFee === null ? "To be confirmed" : formatPrice(storefrontConfig.deliveryFee)}</span></div><div className="flex justify-between gap-4 border-t border-brand-deep/15 pt-4 text-base font-bold text-brand-deep"><span>Total</span><span>{total === null ? "Price pending" : formatPrice(total)}</span></div></div><div className="grid gap-3 bg-brand-sage/45 p-4 text-xs text-brand-ink/70"><p><span className="font-bold text-brand-deep">First delivery:</span> {dateLabel}</p><p><span className="font-bold text-brand-deep">Deliver to:</span> {customer.name}, {customer.address}, {customer.pin}</p></div><div className="mt-5 flex gap-3 border border-brand-green/25 bg-brand-sage/45 p-4 text-sm text-brand-deep"><CircleHelp className="mt-0.5 size-4 shrink-0" /><p>Payment is being connected next. This review does not create an order or charge you.</p></div></div>; }
