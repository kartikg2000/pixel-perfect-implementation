  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-brand-cream text-brand-ink pb-20 md:pb-0">
      <header className="absolute inset-x-0 top-0 z-30 border-b border-brand-deep/10 bg-brand-cream/90 backdrop-blur-sm">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-10">
          <a href="#top" className="flex items-center gap-3" aria-label="My Healthy Platter home">
            <LogoMark />
            <span className="max-w-[130px] text-[11px] font-bold leading-[1.05] tracking-[0.14em] text-brand-deep sm:max-w-none sm:text-xs">
              MY HEALTHY<br className="sm:hidden" /> PLATTER
            </span>
          </a>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 flex gap-2 border-t border-brand-deep/15 bg-brand-cream/95 p-3 shadow-soft backdrop-blur-md sm:hidden">
        <a href={whatsappMessage("Hi My Healthy Platter, I need help with my order.")} target="_blank" rel="noreferrer" className="inline-flex h-11 w-12 shrink-0 items-center justify-center border border-brand-deep/20 text-brand-deep" aria-label="WhatsApp support"><MessageCircle className="size-5" /></a>
...
function LogoMark() {
  return <div className="relative flex size-10 items-center justify-center rounded-full border-2 border-brand-deep"><span className="absolute -top-1.5 left-2.5 h-2.5 w-1.5 rotate-[-30deg] rounded-full bg-brand-green" /><span className="font-display text-lg font-bold text-brand-deep">M</span></div>;
}

function FoodStillLife() {
  return <div className="relative mx-auto aspect-[0.96] w-full max-w-[560px] overflow-hidden border border-brand-deep/10 bg-brand-sage/55 p-5 sm:p-8"><div className="absolute right-6 top-6 z-10 border border-brand-deep/15 bg-brand-cream/85 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.15em] text-brand-deep">Fresh from the kitchen</div><div className="relative flex h-full items-center justify-center"><div className="absolute bottom-0 left-1/2 h-[14%] w-[85%] -translate-x-1/2 rounded-[50%] bg-brand-deep/10 blur-xl" /><div className="relative mt-8 grid w-[85%] grid-cols-2 gap-4 sm:gap-6"><FoodBowl type="fruit" /><FoodBowl type="sprouts" /><FoodBowl type="juice" /><FoodBowl type="veggies" /></div></div><p className="absolute bottom-5 left-5 text-xs font-bold uppercase tracking-[0.16em] text-brand-deep/60 sm:bottom-8 sm:left-8">Breakfast, prepared for you</p></div>;
}
...
function CheckoutReview({ plan, selectedProducts, dateLabel, customer, savings }: { plan: (typeof plans)[number]; selectedProducts: Product[]; dateLabel: string; customer: { name: string; mobile: string; address: string; pin: string; landmark: string; window: string }; savings: ReturnType<typeof getSavings> }) { return <div><CheckoutHeading step="05" title="Review your order" description="Here is everything before payment is connected." /><div className="mt-6 space-y-3 border-y border-brand-deep/15 py-5 text-sm">{selectedProducts.map((product) => <div className="flex justify-between gap-4" key={product.id}><span>{product.name}</span><span className="font-semibold">{formatPrice(product.price)}</span></div>)}<div className="flex justify-between gap-4 border-t border-brand-deep/10 pt-3"><span>{plan.name} · {plan.deliveries} {plan.deliveries === 1 ? "delivery" : "deliveries"}</span><span className="font-semibold">{formatPrice(plan.price)}</span></div>{savings && savings.amount > 0 && <div className="flex justify-between gap-4 text-brand-green"><span>Plan savings</span><span>-{formatPrice(savings.amount)}</span></div>}<div className="flex justify-between gap-4"><span>Delivery fee</span><span>{storefrontConfig.deliveryFee === null ? "To be confirmed" : formatPrice(storefrontConfig.deliveryFee)}</span></div><div className="flex justify-between gap-4 border-t border-brand-deep/15 pt-4 text-base font-bold text-brand-deep"><span>Total</span><span>{plan.price === null ? "Price pending" : formatPrice(plan.price + (storefrontConfig.deliveryFee ?? 0))}</span></div></div><div className="grid gap-3 bg-brand-sage/45 p-4 text-xs text-brand-ink/70"><p><span className="font-bold text-brand-deep">First delivery:</span> {dateLabel}</p><p><span className="font-bold text-brand-deep">Deliver to:</span> {customer.name}, {customer.address}, {customer.pin}</p></div><div className="mt-5 flex gap-3 border border-brand-green/25 bg-brand-sage/45 p-4 text-sm text-brand-deep"><CircleHelp className="mt-0.5 size-4 shrink-0" /><p>Payment is being connected next. This review does not create an order or charge you.</p></div></div>; }
